...
the two are complementary. The primitives mirror `bmoe_cyber`'s decoder skills:
  ...
  caesar   : Y = (X - k) mod 256           (single-byte additive key)
  xor      : Y = X ^ k                     (single-byte XOR key)
  sbox     : Y = inv_perm[X]               (custom substitution alphabet)
  stream   : Y[t] = (X[t]-X[t-1]) mod 256  (rolling/CBC keystream — CONTEXT-dependent)
