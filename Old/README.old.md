...
## POC progress (steps 1–2): toward autonomous routing

The loop architecture is being grown into a POC (runs on Apple-Silicon GPU via MPS).

- **Step 1 — dual-stream loop** (`bmoe_poc.py`): a context view (attention) is carried
  alongside the re-grounded token stream. Validated: a context-dependent skill
  (`delta: X[t]−X[t−1]`) works **only** with the context stream (0.97 vs 0.06 without), and
  unseen compositions still compose zero-shot (~0.97).
- **Step 2 — letting the model route itself** (`bmoe_poc2.py`, `bmoe_poc3.py`): the task is
  given by few-shot **demonstrations** (examples `X→Y`, no expert-id token). Finding: a
  parametric "next-expert" head routes single steps but **cannot infer multi-step chains**,
  because a demonstration shows the net input→output **map**, not its **decomposition** into
  skills — and many chains produce the same map.
- **Step 2.3 — sufficiency critic** (`bmoe_poc4.py`) — ✅ **conclusive.** The model proposes
  chains of its own primitives (shortest first) and a **critic judges "is the goal reached?"**
  by verifying each chain on the demonstrations (`chain(X_demo) == Y_demo`). It **routes
  itself with no router and no expert token**, and reaches **1.00 zero-shot on the unseen
  pair and the unseen triple**. Composing primitives + verifying against the goal resolves
  the map↔decomposition ambiguity.
  *Honest caveats:* the critic here is exact match on the demos and routing is exhaustive
  search over `N^K` chains — fine for few skills; at scale this becomes a **learned/semantic
  "is-this-answer-complete" judge** plus guided/beam search, which likely needs LLM-scale
  experts. That is the next step.
...
- [x] **Dual-stream loop with context** validated, incl. a context-dependent skill
      (`bmoe_poc.py`).
- [x] **Autonomous routing from few-shot demos** via a sufficiency critic — zero-shot to
      unseen compositions, no router, no expert token (`bmoe_poc4.py`).
- [ ] **Scale to an LLM**: replace exact-match critic with a learned/semantic
      "is-this-complete" judge, and exhaustive search with a guided/beam proposer.
